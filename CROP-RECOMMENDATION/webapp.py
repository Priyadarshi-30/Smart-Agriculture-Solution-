...
# img = Image.open(os.path.join(os.getcwd(), "crop.png"))
# try:
#     img = Image.open("crop.png")
#     st.image(img, width=500)  # Display the image
# except FileNotFoundError:
#     st.error("Image file 'crop.png' not found. Please check the file path.")
...
try:
    img = Image.open("crop.png")
    st.image(img, width=500)  # Display the image if successfully loaded
except FileNotFoundError:
    st.error("Image file 'crop.png' not found. Please check the file path.")
    img = None  # Set img to None if the file is not found

# Avoid calling st.image() again if img is None
if img:
    st.image(img, width=500)


df= pd.read_csv('C:/Users/priya/OneDrive/Desktop/Semester VII/Capstone Project [PIP2001]/HarvestHub Project/CROP-RECOMMENDATION/Crop_recommendation.csv')
...
RF_pkl_filename = os.path.join(os.path.expanduser("~"), "Documents", "RF.pkl")
...
if os.path.exists('RF.pkl'):
    st.success("RF.pkl file found. Proceeding to load.")
else:
    st.error("RF.pkl file not found. Please check the file path or ensure the file is created.")
